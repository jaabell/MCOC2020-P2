...
ver_reticulado_3d(ret_D, 
    opciones_nodos = {
        ...
        "factor_amplificacion_deformada": fac,
    },
    opciones_barras = {
...
# -----------------------------------------------------------
# DEFORMADA O DESPLAZAMIENTOS NODALES EN EL RETICULADO
# ANTES DE MODIFICAR LAS GEOMETRIA DE LAS BARRAS


deformada_antes = ret_D.u + ret_L.u


GDL_NODOS = [2,5,8,11,23,26,29,32]

lt = []

for i in GDL_NODOS:
	lt.append(deformada_antes[i])

lt2 = []

for j in lt:
	lt2.append(round(j,5))
	
print (f"la deformada mas grande antes de los cambios es de {min(lt2)} m\n")
...
maximos = [ abs(max(f_1)), abs(min(f_1)) , abs(max(f_2)),  abs(min(f_2)) ]
Fu = max(maximos)
...
# -----------------------------------------------------------
# DEFORMADA O DESPLAZAMIENTOS NODALES EN EL RETICULADO
# DESPUES DE MODIFICAR LAS GEOMETRIA DE LAS BARRAS


deformada_desp = ret_D.u + ret_L.u
...
print (f"la deformada mas grande despues de los cambios es de {min(lt2)} m")
